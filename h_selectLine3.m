function h_selectLine3

h_unSelectLine3; % h_selectCurrentPlot3 is also called inside FV_unSelectLine
...
if isfield(UserData,'timeLastClick') && etime(t0,UserData.timeLastClick) < 0.4
    ...
    if dist2>3*dist1 && dist3>3*dist1
        ...
        if exist(fname, 'file')
            ...
            h_openFile3(guihandles(UserData.parentGUIFigureHandle), fileInfo.name);
        else
            h_openFile3(guihandles(UserData.parentGUIFigureHandle), UserData.activeGroup.groupFiles(I).name);
        end
    end
end
