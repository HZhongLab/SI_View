function h_frameAnalysisGroupCalc3(handles)
...
%         h_img3.(currentStructName).oldimg = h_getOldImg3(handles);
        ...
        analysisFilename = fullfile(pname,'Analysis',[fname,'_frameAnalysis3_A', num2str(analysisNum), '.mat']);
        
        if ~(ismember(grpCalcOpt, [3, 5]) && exist(analysisFilename, 'file'))% 3 and 5 are un-analyzed only.
            h_openFile3(handles, filename);
            if ismember(grpCalcOpt, [4, 5, 7]) % keep z
            ...
            if ismember(grpCalcOpt, 7) && currentStruct.state.frameAnalysisLoadSettingOpt.value 
                % set z (if option 7) once more as load setting may change it.
                % additional: z are not recorded in state:
                set(handles.zStackStrLow,'String', num2str(zLim(1)));
                set(handles.zStackStrHigh,'String', num2str(zLim(2)));
                h_zStackQuality3(handles);
            ...
            h_executeFrameAnalysisCalc3(handles);
...
h_genericSettingCallback3(handles.frameAnalysisPauseGrpCalc, handles);
